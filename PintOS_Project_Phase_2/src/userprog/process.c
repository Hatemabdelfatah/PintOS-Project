...
process_execute (const char *file_name) 
{
	...
	if (fn_copy == NULL)
		return TID_ERROR;
	if (file_name == NULL) {
		// Handle the error, e.g., return an error code or handle the invalid case
		return TID_ERROR;
	}
	...
	file_name_copy = strtok_r((char *) file_name_copy, " ", &save_ptr);
	...
	// parent process should wait for the child to be loaded successfully 
	sema_down(&thread_current()->sema_child_load);
...
start_process (void *file_name_)
{
	...
	//store child loading status
	parent->child_load_success = success;
...
process_wait (tid_t child_tid ) 
{
	...
	for (struct list_elem* e = list_begin (&cur->children_list); e != list_end (&cur->children_list);
    e = list_next (e))
    {
...
process_exit (void)
{
	...
	// closing all files opened by thread
	for (struct list_elem *e = list_begin(&cur->files_list); e != list_end(&cur->files_list);)
	{
		struct list_elem *next = list_next(e); // Save next element
	...
	// relaesing all locks acquired by thread
	for (struct list_elem *e = list_begin(&cur->locks_acquired); 
	e != list_end(&cur->locks_acquired); ) 
	{
		struct lock *acquired_lock = list_entry(e, struct lock, elem);
		e = list_next(e);  // Advance before removal
		lock_release(acquired_lock);
		list_remove(&acquired_lock->elem);
	}

	// freeing all the child structs allocated by the thread for her children
	for (struct list_elem *e = list_begin(&cur->children_list); 
		e != list_end(&cur->children_list); ) 
		{
			struct child_info *child = list_entry(e, struct child_info, elem);
			e = list_next(e);  // Advance before removing the current element

			list_remove(&child->elem);
			palloc_free_page(child);  // Or free(child) if you used malloc
		}

	/*after realsing and closing every thing awake the waiting parent if he is waiting*/
	if (cur->parent_thread != NULL) {
    	sema_up(&cur->my_info->wait_sema);
    }
